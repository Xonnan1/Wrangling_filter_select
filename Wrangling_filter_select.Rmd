...
After running the r-code the data set provided a set of 8 Batman movies.
...
The following code is in the wrong order which caused the error.
...
After running the code I foun out that the longest movie was the Cure for Insomnia (5220 minutes). While the oldest movie was the Blacksmith scene (1893).
...
After running the code I found out that there are 1248 movies that were released in 1995.
...
After running the above code, I found out that there are 4 movies which match the description of 120 minutes and ratings 8 and higher.
...
After running the code above I found out that there are 3 movies of Harry Potter which includes Chamber of Secrets, Prisoner of Azkaban, and the Sorcerer's Stone. The movie with the highest rating was the Prisoner of Azkaban rating of 7.8 while, the most votes were Sorcerer's Stone.
...
After running the code that the movie with the highest budget was the Wild Wild West with a budget of 170000000 and votes of 19078.
